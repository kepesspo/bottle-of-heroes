#!/usr/bin/env python3
"""patch_9_129.py — IFrame API + 0x0 hidden embed + auto-blacklist non-loading songs"""

with open('index.html', 'r', encoding='utf-8') as f:
    content = f.read()

assert "const APP_VERSION = 'v9.127';" in content
content = content.replace("const APP_VERSION = 'v9.127';", "const APP_VERSION = 'v9.129';")

# ── 1. Remove Deezer/proxy block ──
...
# ── 2. Replace ZeneGame states+effect with IFrame API + blacklist version ──
OLD_AUDIO_STATES = """  const filteredSongs = React.useMemo(() => {
  ...
  const song     = filteredSongs.length > 0 ? filteredSongs[gameIdx % filteredSongs.length] : null;
...
NEW_IFRAME_STATES = """  const filteredSongs = React.useMemo(() => {
    let songs = [...ZENE_SONGS];
    if (selectedEra !== 'all') songs = songs.filter(s => s.era === selectedEra);
    if (selectedGenre !== 'all') songs = songs.filter(s => s.genre === selectedGenre);
    if (songs.length === 0) songs = [...ZENE_SONGS];
    // Filter out blacklisted songs (failed to load previously)
    try {
  ...
  React.useEffect(() => {
    ...
    const skipAndBlacklist = () => {
      // Blacklist this song so it never appears again
...
# ── 5. Fix buttons: loading → !embedReady ──
