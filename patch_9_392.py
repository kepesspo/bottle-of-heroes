#!/usr/bin/env python3
"""v9.392 — Splash: csak fade eltűnés marad, választó eltávolítva"""

with open('index.html', 'r', encoding='utf-8') as f:
    html = f.read()

# ── 1. CSS: exit animációk törlése ────────────────────────────────────────────
...
# ── 2. JS: _hideSplash visszaegyszerűsítve ────────────────────────────────────
...
# ── 3. HomeScreen: splashExit state törlése ───────────────────────────────────
old_state = """  const [splashExit, setSplashExit] = React.useState(() => localStorage.getItem('boh_splash_exit') || 'fade');
...
html = html.replace(old_state, '', 1)

# ── 4. Settings UI: választó törlése ─────────────────────────────────────────
...
html = html.replace("const APP_VERSION = 'v9.391';", "const APP_VERSION = 'v9.392';")

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Done: v9.392 — Csak fade eltűnés marad")
